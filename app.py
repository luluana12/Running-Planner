...
def load_runs(path,date_col, dist_col, time_col,pace_col, unit):
    ...
    if 'Date' not in df.columns or 'Distance' not in df.columns or 'elapsed_time' not in df:
        st.error("Please map the correct column names in the sidebar.")
        return None
    
    #Ensure the internal column names exist (case insensitive)
    df.columns = [c.lower() for c in df.columns] #lowercase all columns to match our internal names
...
# Streamlit UI
# Set up the Streamlit web app with sidebar inputs and main display areas
st.set_page_config(page_title="Runner Planner", page_icon="🏃‍♂️", layout = "wide")
st.title("🏃‍♂️ Runnner Performance & Training Planner (MVP)")

with st.sidebar:
    st.header("Data")
    path = st.text_input("Path to your runs CSV", value = "data/running_data .csv")
    date_col = st.text_input("Date column", value = "date")
    dist_col = st.text_input("Distance column", value = "distance")
    time_col = st.text_input("Elapsed time column", value = "elapsed_time")
    pace_col = st.text_input("Pace column (optional)", value = "")
    unit = st.selectbox("Distance unit", ["miles","km"], index = 0)

    st.header("Race & Goals")
    race_day = st.date_input("Race Day", value=date.today() + timedelta(days=56))
    goal_distance = st.number_input("Goal Distance (miles)", value=13.1, step=0.1)
    goal_pace = st.text_input("Goal Pace (min/mile)", value="09:00")

    st.header("Weekly Structure")
    run_days = st.slider("Running days/week", 3, 7, 5)
    long_run_day = st.selectbox("Long run day", ["Saturday","Sunday"], index=0)
    available_days = st.multiselect("Availability (days you can run)", DAYS, default=["Tuesday","Thursday","Saturday","Sunday"])

    st.header("Strength")
    strength_yes = st.checkbox("Include strength?", value=True)
    strength_days = st.slider("Strength days/week", 0, 4, 2)

    st.header("Constraints")
    vacations = st.text_area("Vacation days (YYYY-MM-DD, ...)", value="")
    b_races = st.text_area("B-races (YYYY-MM-DD:distance, ...)", value="")

st.subheader("1) Load & Inspect History")
runs = load_runs(path, date_col, dist_col, time_col, pace_col, unit)
if runs is None or runs.empty:
    st.info("Load your data to see stats.")
else:
    st.dataframe(runs.tail(20), use_container_width=True)
    total_dist = runs["distance_mi"].sum()
    avg_pace = runs["pace_min_per_mi"].mean()
    col1, col2 = st.columns(2)
    col1.metric("Total Distance", f"{total_dist:.1f} miles")
    col2.metric("Average Pace", min_to_pace_str(avg_pace))

st.subheader("2) Performance Estimate for Next Race")
if runs is not None and not runs.empty:
    est = estimate_next_race(runs, goal_distance, goal_pace)
    st.write(f"**Suggested Race Pace:** {est['suggested_pace_str']} min/mi")
    st.write(f"**Estimated Finish Time:** {est['pred_time_str']}")

st.subheader("3) Training Plan")
if runs is not None and not runs.empty:
    plan = build_plan(
        race_day=race_day,
        goal_distance_miles=goal_distance,
        goal_pace_str=goal_pace,
        run_days_per_week=run_days,
        long_run_day=long_run_day,
        allowed_days=available_days,
        strength_yes=strength_yes,
        strength_days_per_week=strength_days,
        vacations_str=vacations,
        b_races_str=b_races
    )
    st.success(f"Generated plan with {len(plan)} days.")
    st.dataframe(plan, use_container_width=True)
    st.download_button("Download Plan CSV", plan.to_csv(index=False).encode("utf-8"), "training_plan.csv", "text/csv")
